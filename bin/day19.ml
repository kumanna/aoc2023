...
let parse_operation s =
  if String.contains s '<' then
    let rating = String.get s 0 |> char_to_rating in
    match (String.sub s 2 ((String.length s) - 2) |> String.split_on_char ':') with
    | [num;wfs] -> (Less_than (int_of_string num), rating, wfs)
    | _ -> failwith ("Error parsing operation: " ^ s)
  else if String.contains s '>' then
    let rating = String.get s 0 |> char_to_rating in
    match (String.sub s 2 ((String.length s) - 2) |> String.split_on_char ':') with
    | [num;wfs] -> (Greater_than (int_of_string num), rating, wfs)
    | _ -> failwith ("Error parsing operation: " ^ s)
  else
    (No_comparison, No_rating, s)

let parse_workflow s =
  let wr = Str.regexp "^\\([a-z][a-z]*\\)[{]\\(..*\\)[}]$" in
  let _ = Str.search_forward wr s 0 in
  (Str.matched_group 1 s, Str.matched_group 2 s)

let operation_table = Hashtbl.create 16
...
let process_part part =
  let rec process_part_helper workflows part =
    match workflows with
    | [] -> failwith "No workflows!"
    | current_workflow::rest ->
      match current_workflow with
      | (No_comparison, No_rating, "A") -> part
      | (No_comparison, No_rating, "R") -> { x = 0; m = 0; a = 0; s = 0;}
      | (Greater_than value, R_x, next_workflow) -> if part.x > value then process_part_helper (Hashtbl.find operation_table next_workflow) part else process_part_helper rest part
      | (Greater_than value, R_m, next_workflow) -> if part.m > value then process_part_helper (Hashtbl.find operation_table next_workflow) part else process_part_helper rest part
      | (Greater_than value, R_a, next_workflow) -> if part.a > value then process_part_helper (Hashtbl.find operation_table next_workflow) part else process_part_helper rest part
      | (Greater_than value, R_s, next_workflow) -> if part.s > value then process_part_helper (Hashtbl.find operation_table next_workflow) part else process_part_helper rest part
      | (Less_than value, R_x, next_workflow) -> if part.x < value then process_part_helper (Hashtbl.find operation_table next_workflow) part else process_part_helper rest part
      | (Less_than value, R_m, next_workflow) -> if part.m < value then process_part_helper (Hashtbl.find operation_table next_workflow) part else process_part_helper rest part
      | (Less_than value, R_a, next_workflow) -> if part.a < value then process_part_helper (Hashtbl.find operation_table next_workflow) part else process_part_helper rest part
      | (Less_than value, R_s, next_workflow) -> if part.s < value then process_part_helper (Hashtbl.find operation_table next_workflow) part else process_part_helper rest part
      | (No_comparison, _, next_workflow) -> process_part_helper (Hashtbl.find operation_table next_workflow) part
      | _ -> failwith "Incorrect workflow!"
  in
...
let () =
  ...
  |> List.map (fun (x, y) -> (x, y |> String.split_on_char ',' |> List.map parse_operation))
  |> List.iter (fun (x, y) -> Hashtbl.add operation_table x y);
  Hashtbl.add operation_table "R" [(No_comparison, No_rating, "R")];
  Hashtbl.add operation_table "A" [(No_comparison, No_rating, "A")];
  part_ratings
  |> List.map (fun x -> String.sub x 1 ((String.length x) - 2))
  |> List.map (String.split_on_char ',')
  |> List.map (fun x -> List.map (fun y -> (String.sub y 2 ((String.length y) - 2)) |> int_of_string) x)
  |> List.map (fun x ->
      match x with
      | [x;m;a;s] -> { x ; m ; a ; s}
      | _ -> failwith "Error parsing parts!")
